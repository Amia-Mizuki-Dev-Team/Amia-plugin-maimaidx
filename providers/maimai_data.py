...
def _server_filters(query) -> list[tuple[str, str]] | None:
    """Translate the shared Core query to Diving-Fish filter parameters.

    The provider still applies ``record_matches_query`` after normalization;
    the server-side values are an optimization and must never widen the
    result set when an older server ignores one of them.
    """
    if query is None:
        return None
    params: list[tuple[str, str]] = []
    if query.min_constant is not None or query.max_constant is not None:
        left = "" if query.min_constant is None else str(query.min_constant)
        right = "" if query.max_constant is None else str(query.max_constant)
        params.append(("ds", f"{left}..{right}"))
    params.extend(("version", str(version)) for version in query.versions)
    params.extend(("type", str(chart_type)) for chart_type in query.chart_types)
    params.extend(("level_index", str(index)) for index in query.difficulty_indexes)
...
def _record_to_core(record, *, source: str, query=None):
    ...
    raw_song_id = _value(record, "song_id", "id", "music_id")
...
class MaimaidxDataProvider:
    ...
    async def get_player_records(self, identity, query=None):
        ...
        if qq is None:
            return []
        subject = maiApi.oauth_subject(qqid=qq)
