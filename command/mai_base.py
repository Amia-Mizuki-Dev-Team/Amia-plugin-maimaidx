...
def _bound_qq(event: MessageEvent) -> int | None:
    """Resolve the real QQ through qbind; never hash an adapter id."""
    real = get_real_qq(str(event.user_id))
    return int(real) if real and str(real).isdigit() else None


async def _require_bound(matcher, event: MessageEvent) -> int:
    qqid = _bound_qq(event)
    if qqid is None:
        await matcher.finish(
            "我还没把你的平台身份绑定到真实 QQ，先完成 qbind 绑定后再查询个人数据。",
            reply_message=True,
        )
    return qqid
...
async def _(bot: Bot, event: MessageEvent):
    ...
    """
    qqid = await _require_bound(user_profile, event)
    try:
        bind = await maiApi.check_bind_status(qqid)
    except Exception as exc:
        log.exception("mai状态 绑定状态查询失败")
        await send_error_with_diagnostic(user_profile, exc, "MAI", context="mai状态")
        return
...
async def _(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        log.exception("mai曲线 数据查询失败")
        await send_error_with_diagnostic(render_curve, exc, "MAI", context="mai曲线")
        return
    if not curves:
