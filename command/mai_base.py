...
async def _(bot: Bot, event: MessageEvent):
    ...
    md_help = (
        ...
        "**⚔️ 社交竞技模块**\n"
        "• `mai对决 @用户` : 对比两人B50并生成格斗PK海报\n"
        "• `今日课题` : 查看今日群挑战课题及成绩排行榜大图\n"
        "• `提交课题` : 同步最近游玩成绩并提交今日课题榜\n\n"
        "**🔍 曲目与进度检索**\n"
        "• `查歌 <关键词>` : 全局模糊检索歌曲名\n"
        "• `id <曲目ID>` : 调取目标谱面核心底标参数\n"
        "• `<版本><目标>完成表` : 查看牌子进度视觉化网格墙\n\n"
        "**🎮 趣味互动游戏**\n"
        "• `猜歌` / `猜曲绘` / `听歌猜歌` / `别名猜歌` : 开启群内趣味猜曲\n\n"
        "**⚙️ 账户与路由中心**\n"
    ...
    inline_keyboard = {
        "rows": [
            {
                "buttons": [
                    {"id": "b50", "render_data": {"label": "📊 我的 B50", "style": 1}, "action": {"type": 2, "permission": {"type": 2}, "data": "b50", "enter": True}},
                    {"id": "profile", "render_data": {"label": "👤 个人状态", "style": 1}, "action": {"type": 2, "permission": {"type": 2}, "data": "mai状态", "enter": True}},
                    {"id": "duel", "render_data": {"label": "⚔️ 战力对决", "style": 1}, "action": {"type": 2, "permission": {"type": 2}, "data": "mai对决", "enter": True}}
                ]
            },
            {
                "buttons": [
                    {"id": "challenge", "render_data": {"label": "🏆 今日课题", "style": 1}, "action": {"type": 2, "permission": {"type": 2}, "data": "今日课题", "enter": True}},
                    {"id": "guess", "render_data": {"label": "🎮 猜歌游戏", "style": 1}, "action": {"type": 2, "permission": {"type": 2}, "data": "猜歌", "enter": True}},
                    {"id": "voice", "render_data": {"label": "🎵 听歌猜曲", "style": 1}, "action": {"type": 2, "permission": {"type": 2}, "data": "听歌猜歌", "enter": True}}
                ]
            },
            {
                "buttons": [
                    {"id": "to_lx", "render_data": {"label": "❄️ 切至落雪", "style": 2}, "action": {"type": 2, "permission": {"type": 2}, "data": "切换数据源 落雪", "enter": True}},
                    {"id": "to_fi", "render_data": {"label": "🔮 切至水鱼", "style": 2}, "action": {"type": 2, "permission": {"type": 2}, "data": "切换数据源 水鱼", "enter": True}}
                ]
            }
        ]
    }
    ...
    if is_official_bot(bot.self_id):
        md_data = {
            "markdown": {"content": md_help},
            "keyboard": {"content": inline_keyboard}
        }
...
async def _(bot: Bot, event: MessageEvent):
    ...
    inline_keyboard = {
        "rows": [
            {
                "buttons": [
                    {"id": "set_lxns", "render_data": {"label": "❄️ 默认设为落雪", "style": 2}, "action": {"type": 2, "permission": {"type": 2}, "data": "切换数据源 落雪", "enter": True}},
                    {"id": "set_fish", "render_data": {"label": "🔮 默认设为水鱼", "style": 2}, "action": {"type": 2, "permission": {"type": 2}, "data": "切换数据源 水鱼", "enter": True}}
                ]
            },
            {
                "buttons": [
                    {"id": "v_curve", "render_data": {"label": "📈 趋势折线走势图", "style": 1}, "action": {"type": 2, "permission": {"type": 2}, "data": "mai曲线", "enter": True}}
                ]
            },
            {
                "buttons": [
                    {"id": "lnk_lx", "render_data": {"label": "🌐 落雪主页传送", "style": 0}, "action": {"type": 0, "permission": {"type": 2}, "data": "https://maimai.lxns.net/user/profile?tab=profile"}},
                    {"id": "lnk_fi", "render_data": {"label": "🌐 水鱼主页传送", "style": 0}, "action": {"type": 0, "permission": {"type": 2}, "data": "https://www.diving-fish.com/maimaidx/prober/"}}
                ]
            }
        ]
    }
...
async def _(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        from ..libraries.maimaidx_player_score import draw_github_heatmap
        await render_heatmap.finish(draw_github_heatmap(heat_data, player_data.get('name', '未知')), reply_message=True)
