...
class _UnavailableEconomy:
    """Keep DX Pass commands loadable when the optional economy plugin is absent."""

    async def get_public_balance(self, _qqid):
        return {"status": False, "code": "unavailable"}

    async def try_spend_public(self, *_args, **_kwargs):
        return {"status": False, "code": "unavailable"}

    async def refund_public(self, *_args, **_kwargs):
        return {"status": False, "code": "unavailable"}
...
try:
    ...
    # The full bot normally supplies this optional companion.  Do not let an
    # isolated maimaidx load lose all score commands just because DX Pass's
    # payment provider is not present.
    economy_plugin = _UnavailableEconomy()
DXPASS_PRICE = 200
_PENDING_TTL = 300
