...
from ..config import *
from .image import DrawText, image_to_base64, music_picture, get_cover_image, get_asset_image
...
class ScoreBaseImage:
    ...
    def whiledraw(self, data: Union[List[ChartInfo], List[PlayInfoDefault], List[PlayInfoDev]], dx: bool, height: int = 0) -> None:
        ...
        for num, info in enumerate(data):
            ...
            cover = get_cover_image(info.song_id, (75, 75))
            type_name = 'SD' if info.type.lower() == 'standard' else info.type.upper()
            version = get_asset_image(f'{type_name}.png', (37, 14))
            rate_filename = f'UI_TTR_Rank_{score_Rank_l[info.rate]}.png' if info.rate.islower() else f'UI_TTR_Rank_{info.rate}.png'
            rate = get_asset_image(rate_filename, (63, 28))
            ...
            if info.fc:
                fc = get_asset_image(f'UI_MSS_MBase_Icon_{fcl[info.fc]}.png', (34, 34))
                self._im.alpha_composite(fc, (x + 154, y + 77))
            if info.fs:
                fs = get_asset_image(f'UI_MSS_MBase_Icon_{fsl[info.fs]}.png', (34, 34))
            ...
            if _music and len(_music.charts) > info.level_index:
                ...
                if dxnum:
                    self._im.alpha_composite(get_asset_image(f'UI_GAM_Gauge_DXScoreIcon_0{dxnum}.png', (47, 26)), (x + 217, y + 80))
