...
from .maimaidx_model import Stats
from .score_line import calculate_score_line
...
async def music_global_data(music: Music, level_index: int) -> MessageSegment:
    if level_index < 0 or level_index >= len(music.ds):
        raise ValueError("该曲目没有对应的难度。")
    stats_list = getattr(music, 'stats', None) or []
    stats = stats_list[level_index] if level_index < len(stats_list) else None
    if isinstance(stats, dict):
        stats = Stats(**stats)
        stats_list[level_index] = stats
        music.stats = stats_list
    if stats is None:
                        ...
                        for s in song_stats:
                            ...
                            if fit_val is not None:
                                ...
                                if diff_val < best_diff:
                                    ...
                                    best_stat = s
                        if best_diff < 0.2:
                            aligned_stats[idx] = Stats(**best_stat)
                    music.stats = aligned_stats
                    stats = aligned_stats[level_index] if level_index < len(aligned_stats) else None
            except Exception as e:
                log.error(f"Failed to load chart stats: {e}")
    if not stats or not stats.fc_dist or not stats.dist:
...
class DrawScore(ScoreBaseImage):
    ...
    def __init__(self, image: Image.Image = None, source: SourceName | str = "merged") -> None:
    ...
    def draw_rise(self, sd: List[RiseScore], sd_score: int, dx: List[RiseScore], dx_score: int) -> Image.Image:
        ...
        # This variant uses an 800px footer strip rather than the normal
        # 1000px strip, so reserve a narrower inner area for the full credit.
        draw_attribution(
            self._sy,
            self._im.size[0],
            height - 76,
    ...
    def draw_plan(
        ...
        from .attribution import draw_attribution
        draw_attribution(self._sy, self._im.size[0], self._im.size[1] - 20, self.source, self.text_color)
        return self._im
...
def plate_message(
    ...
    if len(music_list) > 10:
        return MessageSegment.image(text_to_bytes_io(result.strip()))
...
async def player_plate_data(
    ...
    return result if isinstance(result, MessageSegment) else MessageSegment.text(result)
...
async def level_process_data(
    ...
    """
    try:
        raw_records = await maiApi.query_user_get_dev(qqid=qqid, username=username)
        if isinstance(raw_records, dict):
            obj = raw_records.get("records", raw_records.get("data", []))
        else:
            obj = raw_records
        ...
        if category == 'default':
            ...
            dp = DrawScore(image, source="diving-fish")
            ...
            lendata = len(data)
            end_page_num = max(1, (lendata + 79) // 80)
            if page < 1 or page > end_page_num:
                raise PageOutOfRangeError(end_page_num)
...
async def level_achievement_list_data(
    ...
    try:
        ...
        raw_records = await maiApi.query_user_get_dev(qqid=qqid, username=username)
        if isinstance(raw_records, dict):
            data = raw_records.get("records", raw_records.get("data", []))
        else:
            data = raw_records
        if not isinstance(data, list):
            raise MaimaiDataFormatError()
        for _d in data:
            if not getattr(_d, "ds", 0):
                music = mai.total_list.by_id(str(_d.song_id))
                if music and _d.level_index < len(music.ds):
                    _d.ds = music.ds[_d.level_index]
            if not getattr(_d, "ra", 0) and getattr(_d, "ds", 0):
        ...
        sc = DrawScore(image, source="diving-fish")
...
def score_line_data(music: Music, level_index: int, line: float) -> str:
    """Calculate the upstream-style TAP GREAT and BREAK 50 line."""
    return calculate_score_line(music, level_index, line, diffs)
...
async def player_score_data(
    ...
    for record in records:
        index = int(getattr(record, 'level_index', -1))
...
async def rating_ranking_data(name: Optional[str], page: Optional[int]) -> MessageSegment:
    ...
    try:
        ...
        log.error(traceback.format_exc())
        raise
    return data if isinstance(data, MessageSegment) else MessageSegment.text(data)
