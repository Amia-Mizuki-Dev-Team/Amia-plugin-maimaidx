...
from ..config import SHANGGUMONO, Path, coverdir, log, maidir

# 全局字体和图片缓存，极大提升老旧 CPU 上的绘图性能
_font_cache = {}
_cover_cache = {}
_asset_cache = {}


def get_cover_image(music_id: Union[int, str], size: Tuple[int, int]) -> Image.Image:
    """获取缓存的已缩放曲绘，防止在循环中重复解码和缩放图片"""
    key = (str(music_id), size)
    if key not in _cover_cache:
        path = music_picture(music_id)
        try:
            img = Image.open(path).convert('RGBA').resize(size)
        except Exception as e:
            log.warning(f"读取/缩放曲绘 {music_id} 失败: {e}，将使用默认占位图")
            try:
                fallback_path = coverdir / '0.png'
                if fallback_path.exists():
                    img = Image.open(fallback_path).convert('RGBA').resize(size)
                else:
...
def get_asset_image(filename: str, size: Tuple[int, int]) -> Image.Image:
    """获取缓存的已缩放UI静态资产"""
    key = (filename, size)
    if key not in _asset_cache:
        path = maidir / filename
        try:
            img = Image.open(path).convert('RGBA').resize(size)
        except Exception as e:
            log.warning(f"读取/缩放静态资产 {filename} 失败: {e}")
            img = Image.new('RGBA', size, (0, 0, 0, 0))
...
class DrawText:
    ...
    def _get_font(self, size: int) -> ImageFont.FreeTypeFont:
        ...
        if key not in _font_cache:
            _font_cache[key] = ImageFont.truetype(self._font, size)
        return _font_cache[key]

    def get_box(self, text: str, size: int) -> Tuple[float, float, float, float]:
        return self._get_font(size).getbbox(text)
    ...
    def draw(
        ...
        if char_spacing:
            ...
            baseline_offset = -(full_bbox[1] + full_bbox[3]) // 2 + 7
            if anchor == 'lt':
                pass  # lt → ls 无需偏移
            elif anchor in ('lm',):
                y -= baseline_offset
